...
with tab_verify:
    ...
    if run_btn:
        ...
        st.subheader("Formal Verification Report")
        for res in report.results:
            if res.status == VerificationResultStatus.VIOLATION_DETECTED:
                st.markdown(f"""
                <div class="card-violation">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <h4 style="color: #ef4444; margin: 0;">❌ Invariant Violation: {res.invariant_name} ({res.invariant_id})</h4>
                        <span style="background-color: #ef4444; color: white; padding: 2px 8px; border-radius: 4px; font-size: 0.8rem; font-weight: bold;">Z3 SAT ({res.solver_time_ms} ms)</span>
                    </div>
                    <p style="margin-top: 8px; color: #cbd5e1;"><b>Target Clauses:</b> {', '.join(res.involved_clauses)}</p>
                </div>
                """, unsafe_allow_html=True)

                if res.counterexample:
                    st.markdown("**🔍 Mathematical Counterexample Generated by Z3:**")
                    chips_html = "".join([f'<span class="metric-chip"><b>{k}</b>: {v}</span>' for k, v in res.counterexample.items()])
                    st.markdown(chips_html, unsafe_allow_html=True)

                if res.explanation:
                    st.info(f"**Legal Diagnosis:**\n\n{res.explanation}")

                if res.suggested_redline:
                    st.success(f"**Recommended Redline Amendment:**\n\n```text\n{res.suggested_redline}\n```")

            elif res.status == VerificationResultStatus.PROVED_VALID:
                st.markdown(f"""
                <div class="card-valid">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <h4 style="color: #22c55e; margin: 0;">✅ Formally Proved Sound: {res.invariant_name} ({res.invariant_id})</h4>
                        <span style="background-color: #22c55e; color: white; padding: 2px 8px; border-radius: 4px; font-size: 0.8rem; font-weight: bold;">Z3 UNSAT ({res.solver_time_ms} ms)</span>
                    </div>
                    <p style="margin-top: 8px; color: #cbd5e1;"><b>Target Clauses:</b> {', '.join(res.involved_clauses)}</p>
                    <p style="color: #94a3b8; font-size: 0.9rem;">Mathematical guarantee: No counterexample exists in the domain. The invariant is unconditionally satisfied.</p>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.warning(f"⚠️ {res.invariant_name}: {res.explanation} (Status: {res.status})")
